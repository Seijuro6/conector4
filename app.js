document.addEventListener("DOMContentLoaded", () => {
  ...
  function checkBoard() {
    //now take the 4 values in each winnibgArrays and plug them into de squares
    for (let y = 0; y < winningArrays.length; y++) {
      const square1 = squares[winningArrays[y][0]];
      const square2 = squares[winningArrays[y][1]];
      const square3 = squares[winningArrays[y][2]];
      const square4 = squares[winningArrays[y][3]];
      //now check tose arrays to see if the all have the class of player-one
      if (
  ...
  for (let i = 0; i < squares.length; i++) {
    //ad onclick to each square in your grid
    squares[i].onclick = () => {
      //if the square below your current squareis taken, you can go ontop of it
      if (
